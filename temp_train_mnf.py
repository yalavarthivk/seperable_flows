...
def create_dataloaders(task, args: argparse.Namespace):
    ...
    train_loader, val_loader, test_loader, eval_loader = data_loader.data_loaders(
        task, args
    )
...
class Trainer:
    ...
    def evaluate_for_marginals_individually(
        self, eval_loader
        ...
        with torch.no_grad():
            for batch in eval_loader:
                ...
                for k in range(nqueries):
                    ...
                    # Forward pass
                    self.optimizer.zero_grad()
                    self.model(tobs, cobs, mobs, xobs, tq, cq, mq_inst)

                    # Compute loss
                    _, mnll = compute_likelihood_losses(self.model, y_inst, mq_inst)
                    additional_metrics = compute_additional_metrics(
                        model=self.model, y=y_inst, mq=mq_inst, n_samples=100
                    )

                    # Accumulate statistics
                    total_mnll += mnll.item()
                    total_mse += additional_metrics.mse.item()
                    total_robust_mse += additional_metrics.robust_mse.item()
                    total_crps += additional_metrics.crps.item()
...
def main():
    ...
    train_loader, valid_loader, test_loader, eval_loader = create_dataloaders(
        task, args
    )
    ...
    indi_likelihood_metrics, indi_energy_metrics, indi_point_metrics = (
        trainer.evaluate_for_marginals_individually(eval_loader)
    )

    eval_time = time.time() - start_time

    logger.info("Final Results:")
    logger.info("Test mNLL individual: %.6f", indi_likelihood_metrics["mnll"])
    logger.info("Test mse individual: %.6f", indi_point_metrics["mse"])
    logger.info("Test robust mse individual: %.6f", indi_point_metrics["robust_mse"])
    logger.info("Test crps individual: %.6f", indi_energy_metrics["crps"])
    logger.info("Evaluation Time: %.2fs", eval_time)
