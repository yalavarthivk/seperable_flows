    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for batch in data_loader:
                ...
                tx, cx, x, mx, tq_full, cq_full, y_full, mq_full = batch_

                # Forward pass
                self.optimizer.zero_grad()
                # pdb.set_trace()
                for i in range(tq_full.shape[1]):
                    tq = tq_full[:, i : i + 1]
                    cq = cq_full[:, i : i + 1]
                    y = y_full[:, i : i + 1]
                    mq = mq_full[:, i : i + 1]
                    ...
                    mse = self.model.mse(y, mq)
                    crps = self.model.crps(y, mq)
                    total_mnll += mnll.item()
                    total_mse += mse.item()
                    total_crps += crps.item()
                total_samples += mq_full.sum().item()

        return (
            total_mnll / total_samples,
            total_crps / total_samples,
...
def main():
    ...
    test_loader = create_dataloaders(task, args.fold, args.batch_size)
    ...
    # Initialize trainer
    evaluater = Marginals_evaluater(model, args, optimizer, scheduler, device, logger)

    # Generate experiment ID and model path
    logger.info("Starting evaluation for mNLL with experiment ID: %d", experiment_id)
    model_path = output_dir / f"{args.dataset}_fold{args.fold}_{args.seed}.pt"
    evaluater.load_checkpoint(str(model_path))
    start_time = time.time()

    # evaluate
    test_mNLL, test_crps, test_mse = evaluater.evaluate(test_loader)

    eval_time = time.time() - start_time

    logger.info(
        "Test mNLL: %.6f | Test MSE: %.6f | Test crps: %.6f | Time: %.2fs",
        test_mNLL,
        test_mse,
        test_crps,
        eval_time,
