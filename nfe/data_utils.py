...
def tsdm_sparse_collate(batch, sparsity):
    context_x = []
    context_vals = []
    context_mask = []
    target_vals = []
...
def tsdm_sparse_collate_val(batch, sparsity):
    context_x = []
    target_x = []
    context_vals = []
    context_mask = []
...
def tsdm_time_sparse_collate(batch, sparsity):
    context_x = []
    context_vals = []
    context_mask = []
    target_vals = []
    ...
    for i, sample in enumerate(batch):
        ...
        torch.manual_seed(mask_x.sum()+mask_y.sum())
        n_select = int(max(1, (1-sparsity)*len(mask_x_inds[0])))
        select_indices = torch.randperm(len(mask_x_inds[0]))[:n_select].sort()[0]
        mask_x_inds = mask_x_inds[0][select_indices]
...
def tsdm_time_sparse_collate_val(batch, sparsity):
    context_x = []
    target_x = []
    context_vals = []
    context_mask = []
