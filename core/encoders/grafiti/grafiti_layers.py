...
def batch_flatten(x_list, mask):
    ...
    observed_counts = m_flat.bool().sum(dim=1).to(torch.int64)
    ...
    for x in x_list:
        ...
        if mask_indices.sum() != observed_values.shape[0]:
            pdb.set_trace()
...
class grafiti_(nn.Module):
    ...
    def __init__(
        ...
        self.output_embed = nn.Linear(nkernel, nkernel * n_gaussians)
        self.relu = nn.ReLU()
    ...
    def forward(
        ...
        flattened = batch_flatten(
            [t_inds, values, obs_mask, target_mask, c_inds, mask_bool], mask
        )
        t_inds_f, obs_vals, obs_mask_f, tgt_mask_f, c_inds_f, mask_f = (
        ...
        # Final output layer
        output_embed = self.relu(self.output_embed(edge_emb))
        query_embed = hembed(output_embed, tgt_mask_f)  # (B, K, M))
        history_embed = hembed(edge_emb, obs_mask_f)  # (B, K, M*G))

        return query_embed, history_embed
