...
def profiti_collate_fn(batch: list[Sample]) -> Batch:
    ...
    if len(batch_tobs) == 0:
        # Return an empty batch with proper structure
        # You need to determine appropriate default dimensions
        return Batch(
            tobs=torch.empty(0, 0),
            cobs=torch.empty(0, 0, dtype=torch.long),
            xobs=torch.empty(0, 0),
            mobs=torch.empty(0, 0),
            tq=torch.empty(0, 0),
            cq=torch.empty(0, 0, dtype=torch.long),
            y=torch.empty(0, 0),
            mq=torch.empty(0, 0),
        )
